ItemGrowUp = ItemGrowUp or class("ItemGrowUp",function()
    return cc.Node:create()
end)
local GOODS_NUM = 4

function ItemGrowUp:ctor()
    self._iconGoods = {}
    self._bgGoods = {}
    self._txtGoodsNum = {}
    self._initPosX = 167
...
function ItemGrowUp:create()
    local node = ItemGrowUp:new()
    node:initItem()
    return node
end

function ItemGrowUp:initItem()
    ...
    self._btnGoto:addTouchEventListener(function(target,type)
        if type == 2 then
            ...
            if zzd.TaskData.arrTypeTitle[self._group.Type] == "TipsFrame" then
                _G[zzd.TaskData.arrTypeTitle[self._group.Type]]:create("功能尚未开放")

            elseif zzd.TaskData.arrTypeTitle[self._group.Type] == "Copy" then
                local copy = zzm.CopySelectModel:getCopyData(self._group.Finish1)
                if copy then
                    ...
                    zzc.CopySelectController:AutoOpenCopyPage(self._group.Finish1)
                    zzc.TaskController:closeLayer()
                else
                    dxyFloatMsg:show("副本尚未解锁")
                end
            elseif zzd.TaskData.arrTypeTitle[self._group.Type] == "LV" then
                zzc.CopySelectController:OpenCopyPage() 
                zzc.TaskController:closeLayer()
            elseif zzc[zzd.TaskData.arrTypeGoto[self._group.Type]] then
            
                if enFunctionType[zzd.TaskData.openType[self._group.Type]] then
                    if zzc.GuideController:checkFunctionTips(enFunctionType[zzd.TaskData.openType[self._group.Type]]) == false then
                        return
                    end
                end
            
                local layer = zzc[zzd.TaskData.arrTypeGoto[self._group.Type]]:showLayer()
                if zzd.TaskData.scrollPage[self._group.Type] then
...
--                dxyFloatMsg:show("")
    ...
    self._btnGet:addTouchEventListener(function(target,type)
        if type == 2 then
            ...
            zzc.TaskController:registerGetGrowUp(self._group.Id)
        end
    end)
    
end

function ItemGrowUp:update(data)
    self.m_data = data
    ...
    self._group = TaskConfig:getGrowUpById(self.m_data.Id)
--    self._rewards = TaskConfig:getRewardsById_GrowUp(self.m_data.Id)
    ...
    self._txtTitle:setString(self._group.Name)
    self._txtInt:setString(self._group.Introduction)
    ...
    if state == 0 then
        ...
        for j=1,#zzd.TaskData.arrNoNeedType do
            if self._group.Type == zzd.TaskData.arrNoNeedType[j] then
                self._btnGet:setVisible(false)
                self._btnGoto:setVisible(false)
...
function ItemGrowUp:findRes(type)
...
function ItemGrowUp:HideRes(target,text,res)
    if res then
...
function ItemGrowUp:HideGoods(data)
    local i = 1

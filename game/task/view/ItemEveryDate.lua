ItemEveryDate = ItemEveryDate or class("ItemEveryDate",function()
    return cc.Node:create()
end)
local GOODS_NUM = 4

function ItemEveryDate:ctor()
    self._iconGoods = {}
    self._bgGoods = {}
    self._txtGoodsNum = {}
    self._initPosX = 167
...
function ItemEveryDate:create()
    local node = ItemEveryDate:new()
    node:initItem()
    return node
end

function ItemEveryDate:initItem()
    ...
    self._txtLively = self._csb:getChildByName("txtLively")
    ...
    self._btnGoto:addTouchEventListener(function(target,type)
        if type == 2 then
            ...
            if zzd.TaskData.arrTypeTitle[self._daily.Type] == "TipsFrame" then
                _G[zzd.TaskData.arrTypeTitle[self._daily.Type]]:create("功能尚未开放")

            elseif zzd.TaskData.arrTypeTitle[self._daily.Type] == "Copy" then
                local copy = zzm.CopySelectModel:getCopyData(self._daily.Finish1)
                if copy then
                    ...
                    zzc.CopySelectController:AutoOpenCopyPage(self._daily.Finish1)
                    zzc.TaskController:closeLayer()
                else
                    dxyFloatMsg:show("副本尚未解锁")
                end
            elseif zzd.TaskData.arrTypeTitle[self._daily.Type] == "LV" then
                zzc.CopySelectController:OpenCopyPage() 
                zzc.TaskController:closeLayer()
            elseif zzc[zzd.TaskData.arrTypeGoto[self._daily.Type]] then
            
                if enFunctionType[zzd.TaskData.openType[self._daily.Type]] then
                    if zzc.GuideController:checkFunctionTips(enFunctionType[zzd.TaskData.openType[self._daily.Type]]) == false then
                        return
                    end
                end
                
                local layer = zzc[zzd.TaskData.arrTypeGoto[self._daily.Type]]:showLayer()
                if zzd.TaskData.scrollPage[self._daily.Type] then
...
--                dxyFloatMsg:show("")
    ...
    self._btnGet:addTouchEventListener(function(target,type)
        if type == 2 then
            ...
            zzc.TaskController:registerGetEveryDate(self._daily.Id)
        end
    end)
    
end

function ItemEveryDate:update(data)
    self.m_data = data
    ...
    self._btnNotyet:setVisible(false)
    self._txtLively:setVisible(false)

    self._daily = TaskConfig:getDailyById(self.m_data.Id)
    ...
    self:HideRes(self._ks,self._txtKS,self:findRes(16)) --坤石
    self:HideRes(self._qs,self._txtQS,self:findRes(17)) --乾石
    self:HideRes(self._lingqi,self._txtLingqi,self:findRes(5)) --灵气
    
    self:HideGoods(self._rewards)
    self._txtTitle:setString(self._daily.Name)
    self._txtInt:setString(self._daily.Introduction)
    if self._daily.Lively ~= 0 then
        self._txtLively:setString("活跃度:+"..self._daily.Lively)
        self._txtLively:setVisible(true)
    end
    ...
    if state == 0 then
        ...
        for j=1,#zzd.TaskData.arrNoNeedType do
            if self._daily.Type == zzd.TaskData.arrNoNeedType[j] then
                self._btnGet:setVisible(false)
                self._btnGoto:setVisible(false)
...
function ItemEveryDate:findRes(type)
...
function ItemEveryDate:HideRes(target,text,res)
    if res then
...
function ItemEveryDate:HideGoods(data)
    local i = 1
